...
export class ServerService {
  ...
  // http will inject into this service 
  constructor(private http: HttpClient) { }
  /*
  getService(): Observable<CustomResponse> {
    return this.http.get<CustomResponse>("http://localhost:8080/server/list");
  }
  is equals as below
  */

  // use <> to cast the type
  ...
  servers2$ = <Observable<Server[]>>
      this.http.get<Server[]>(`${this.apiUrl}/server/list2`)
        .pipe(
          tap(console.log),
          catchError(this.handleError)
        )
  ...
  filter$ = (status: Status, response: CustomResponse) => <Observable<CustomResponse>>
    new Observable<CustomResponse>(
      ...
      subscriber => {
        ...
        // signal that we have finished with what we were emitting
        subscriber.complete();
      }
    )
      .pipe(
